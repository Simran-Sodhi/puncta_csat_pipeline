...
def load_cyto_plane(path, channel_index=1, z_index=0):
    """
...
def postprocess_mask(masks, min_size=0, remove_edges=False):
    """
    Post-process mask by optionally removing small objects and edge-touching objects.
    ...
    min_size : int
        ...
        If 0 or negative, no size filtering is applied.
    remove_edges : bool
        If True, any object touching the image border is removed.

    Returns
    -------
    masks_pp : np.ndarray (Y, X)
        Post-processed mask with consecutive labels (1..N).
    """
    labels = masks.astype(np.int32, copy=False)
  
    # 1) Remove small objects (by area in pixels)
    if min_size is not None or min_size > 0:
    ...
    # 2) Remove objects touching the edge
    if remove_edges:
        h, w = labels.shape
        ...
        edge_labels = np.unique(labels[border_mask])
    ...
    # 3) Relabel to make IDs consecutive again
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Cytoplasm channel index in the input image (default: 1 = second channel)",
    )
    ...
    parser.add_argument(
        ...
        help="Minimum object size in pixels; objects smaller than this "
             "are removed from the mask (default: 0 = no size filtering).",
    )
    ...
    parser.add_argument(
        "--remove-edges",
        action="store_true",
        help="If set, remove any objects touching the image border.",
    )
    ...
    print(f"Found {len(image_paths)} image(s). Initializing Cellpose (cyto3)...")
    ...
    for img_path in image_paths:
        ...
        try:
            ...
            # img_norm = percentile_norm(img2d)
