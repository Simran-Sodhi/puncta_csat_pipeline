...
- Exports a canonical "intensity_for_cs" column = local bg-sub mean,
  suitable for estimating C_sat from puncta presence vs intensity.
...
def make_triptych(img, labels, puncta_mask, has_puncta_map, centroid_map, out_path):
    """
    ...
    # Panel 1: raw cytoplasm channel
    axes[0].imshow(img, cmap="gray")
    axes[0].set_title("Cytoplasm channel (raw)")
    axes[0].axis("off")
...
def main(
    ...
    for cyto_path in sorted(cyto_dir.glob("*.tif")):
        ...
        for lab in range(1, max_label + 1):
            ...
            # compute local background or fall back to global
            if local_bg_pixels_used >= max(50, min_ring_pixels // 4):
                ring_vals = img[ring_mask].astype(np.float32)
                if bg_stat == "median":
                    local_bg = float(np.median(ring_vals))
                else:
                    local_bg = float(np.mean(ring_vals))
            else:
                local_bg = img_bg_global  # fallback

            cyto_mean_bgsub_local = float((cyto_vals_raw - local_bg).mean())
            cyto_mean_norm_local = float(cyto_mean_bgsub_local / p_norm) if p_norm > 0 else 0.0

            # ----- Optional calibration using local bg-sub intensity -----
            if calib_a is not None and calib_b is not None and calib_a != 0:
                conc_estimate_local = (cyto_mean_bgsub_local - float(calib_b)) / float(calib_a)
                conc_estimate_local = float(max(conc_estimate_local, 0.0))
            else:
                conc_estimate_local = None
            ...
            # Canonical column for C_sat fitting
            intensity_for_cs = cyto_mean_bgsub_local
            ...
            rows.append(
                {
                    ...
                    "conc_estimate_local": conc_estimate_local,
